...
function getCountryInfo(country) {
  ...
  db.transaction(function (tx) {
    ...
    if (rs.rows.length > 0) {
      retVal =
        rs.rows.item(0).ISO4217CurrencyCode +
        "|" +
        rs.rows.item(0).ISO4217CurrencyName +
        "|" +
        rs.rows.item(0).ITU_T_TelephoneCode +
        "|" +
        rs.rows.item(0).ISO3166_1_2LetterCode +
        "|" +
        rs.rows.item(0).ISO3166_1_3LetterCode +
        "|" +
        rs.rows.item(0).IANACountryCodeTLD;
    } else {
      retVal = qsTr("Unknown");
    }
  });
...
function readAliases() {
  ...
  db.transaction(function (tx) {
    ...
    for (var i = 0; i < result.rows.length; i++) {
      aliasesDialog.appendCustomCity(
...
function readActiveAliases() {
  ...
  db.transaction(function (tx) {
    ...
    for (var i = 0; i < result.rows.length; i++) {
      mainapp.myAliases +=
        result.rows.item(i).CityInfo + "|" + result.rows.item(i).Alias + "\n";
    }
  });
}
...
function writeAlias(cityinfo, city, alias, displayed) {
  ...
  try {
    db.transaction(function (tx) {
      ...
      tx.executeSql("COMMIT;");
      result = tx.executeSql("SELECT City FROM city_aliases WHERE City=?;", [
        city,
      ]);
    });
